...
export function ProfileControls({
  ...
  return (
    <div className="flex flex-col gap-3.5 md:gap-5">
      <div className="flex flex-col gap-3 md:gap-4 p-4 md:p-5 bg-pio-white border border-pio-border rounded-2xl">
        <div className="flex flex-col gap-1">
          <span className="text-[14px] md:text-[16px] font-extrabold text-pio-ink">Public display</span>
          <span className="text-[11.5px] md:text-[13px] text-pio-muted leading-relaxed">
        ...
        <div className="flex gap-1.5 bg-pio-input rounded-xl p-1">
          {(["FIRST_NAME", "ANONYMOUS"] as const).map((mode) => (
            <button
              ...
              className="pio-tap flex-1 text-center py-2.5 md:py-3 rounded-lg text-[12.5px] md:text-[13.5px] font-bold cursor-pointer"
              style={{
        ...
        <div className="flex items-center gap-2.5 md:gap-3 bg-pio-green-tint rounded-xl p-3 md:p-3.5">
          <div className="w-9 h-9 md:w-10 md:h-10 rounded-full bg-pio-deep flex items-center justify-center text-white shrink-0">
            {displayMode === "ANONYMOUS" ? <ProfileIcon size={16} /> : <span className="text-[15px] font-extrabold">{initial}</span>}
          </div>
          <div className="flex flex-col gap-0.5">
            <span className="text-[10.5px] md:text-[12px] font-semibold" style={{ color: "#6B8478" }}>
              Others will see you as
            </span>
            <span className="text-[13.5px] md:text-[15px] font-extrabold text-pio-ink">
      ...
      <div className="flex items-center justify-between p-3.5 md:p-4.5 bg-pio-white border border-pio-border rounded-2xl">
        <div className="flex items-center gap-3">
          ...
          <span className="text-[13.5px] md:text-[15px] font-bold text-pio-ink">Dark mode</span>
        </div>
        <button
          onClick={() => {
            const next = !darkMode;
      ...
      <Button variant="danger-outline" onClick={() => signOut({ callbackUrl: "/" })} className="md:!py-3.5 md:!text-[14.5px]">
        Log out
      </Button>
    </div>
  );
