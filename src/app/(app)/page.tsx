...
export default async function HomePage() {
  ...
  return (
    <div className="flex flex-col gap-6 md:gap-8 px-4.5 py-4.5 md:px-8 md:py-6">
      ...
      <div className="flex flex-col gap-3.5">
        <div className="flex flex-col gap-0.5">
          <span className="text-[23px] font-extrabold text-pio-ink tracking-tight">
            Hi, {user ? user.name.split(" ")[0] : "there"}
          </span>
          <span className="text-[12.5px] font-semibold text-pio-muted">
            {user ? `${user.course} · ${user.semester}` : "Browse academic material from Amity Noida students."}
          </span>
        </div>
        <div
          className="rounded-[22px] p-5 flex flex-col gap-2.5 relative overflow-hidden"
          style={{ background: "linear-gradient(140deg, var(--pio-hero-from) 0%, var(--pio-hero-to) 100%)" }}
        >
          <div className="absolute -top-10 -right-8 w-35 h-35 rounded-full bg-white/6" style={{ width: 140, height: 140 }} />
          <h1 className="m-0 text-[20px] leading-tight font-extrabold text-white relative tracking-tight">
            Notes, books &amp; files from Amity Noida students.
          </h1>
          <p className="m-0 text-[12.5px] leading-relaxed relative" style={{ color: "var(--pio-hero-sub)" }}>
            Browse what&apos;s listed, or pass on what you no longer need.
          </p>
          <div className="flex gap-2 mt-1 relative">
            <Link href="/browse">
              <span className="pio-tap bg-white text-[#1B4234] font-extrabold text-[12.5px] px-4.5 py-2.5 rounded-full whitespace-nowrap inline-block cursor-pointer">
                Browse
              </span>
            </Link>
            <Link href="/list">
              <span className="pio-tap bg-white/12 text-white border-[1.5px] border-white/35 font-bold text-[12.5px] px-4.5 py-2.5 rounded-full whitespace-nowrap inline-block cursor-pointer">
                List material
              </span>
            </Link>
          </div>
      ...
      {user && !hasSubjects && (
        <div className="bg-pio-white border border-dashed border-pio-border-strong rounded-2xl p-3.5 flex flex-col gap-2.5">
          <span className="text-[13px] font-semibold text-pio-ink-soft">
            Add your subjects for better recommendations.
          </span>
          <div className="flex gap-2">
            <Link href="/profile">
              <Button className="!py-2 !px-3.5 !text-[12px]">Add subjects</Button>
            </Link>
          </div>
        </div>
      )}
      ...
      <Section
        ...
        seeAllHref="/browse"
      >
        <div className="pio-scroller flex gap-3 overflow-x-auto -mx-4.5 px-4.5 md:mx-0 md:px-0 md:grid md:grid-cols-4">
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex items-start justify-between gap-2">
        <div className="flex flex-col gap-0.5">
          <h2 className="m-0 text-[16.5px] font-extrabold text-pio-ink">{heading}</h2>
          {subtext && <span className="text-[11.5px] font-semibold text-pio-muted">{subtext}</span>}
        </div>
        <Link href={seeAllHref} className="text-[12.5px] font-bold text-pio-green whitespace-nowrap pt-0.5">
          See all
        </Link>
      </div>
      {children}
