...
export default async function ListingDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="flex flex-col md:flex-row md:gap-8 md:px-8 md:py-6">
      <div className="flex-1 flex flex-col gap-4 px-4.5 py-4 md:px-0 md:py-0">
        <div className="h-55 md:h-90 rounded-[18px] overflow-hidden bg-pio-photo-bg relative shrink-0" style={{ height: 220 }}>
          {dl.photos[0] ? (
            <img src={dl.photos[0]} alt={dl.title} className="w-full h-full object-cover" />
          ) : (
            <PhotoPlaceholder className="w-full h-full" iconSize={40} />
          )}
        </div>
        ...
        <h1 className="m-0 text-[19px] font-extrabold text-pio-ink tracking-tight">{dl.title}</h1>
        <span className="text-[12.5px] text-pio-muted -mt-2.5">{dl.context}</span>
        <span className="text-[22px] font-extrabold text-pio-ink">{dl.priceLabel}</span>
        ...
        <div className="flex flex-col gap-2 py-3.5 border-t border-b border-pio-border">
        ...
        <div className="flex flex-col gap-1.5">
          <span className="text-[12.5px] font-bold text-pio-ink">Description</span>
          <p className="m-0 text-[12.5px] leading-relaxed text-pio-ink-soft">{dl.description}</p>
        </div>

        {dl.tags.length > 0 && (
          <div className="flex flex-wrap gap-1.5">
            {dl.tags.map((t) => (
              <span key={t} className="text-[11px] font-semibold text-pio-ink-soft bg-pio-input border border-pio-border px-2.5 py-1.5 rounded-full">
                {t}
              </span>
            ))}
          </div>
        ...
        <div className="flex items-center gap-2.5 p-3.5 bg-pio-input rounded-2xl">
          <Avatar name={dl.owner.name} anonymous={dl.owner.anonymous} seed={dl.ownerId} size={38} />
          <div className="flex flex-col gap-0.5 min-w-0 flex-1">
            <span className="text-[13px] font-extrabold text-pio-ink">{dl.owner.name}</span>
            <span className="text-[11px] text-pio-muted">{dl.owner.meta}</span>
          </div>
          {dl.owner.verified && (
            <span className="flex items-center gap-1 text-[10.5px] font-bold text-pio-green bg-pio-green-tint px-2.5 py-1.5 rounded-full shrink-0">
